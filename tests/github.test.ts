...
const unexpected = (method: string) => vi.fn().mockRejectedValue(new Error(`Unexpected ${method} call`));
...
describe('github', () => {
  ...
  describe('findTagFromReleases', () => {
    ...
    it.each([
      ...
      ['an invalid top-level status with a nested response 404', { status: '500', response: { status: 404 } }],
      ...
      await expect(findTagFromReleases(releaser, owner, repo, draftRelease.tag_name)).resolves.toBe(draftRelease);
    ...
    it.each([
      ...
      ['a numeric top-level 500 with a nested response 404', { status: 500, response: { status: 404 } }],
      ...
      await expect(findTagFromReleases(releaser, owner, repo, mockRelease.tag_name)).rejects.toBe(lookupError);
  ...
  describe('listReleaseAssets', () => {
    ...
    it('stops after exhausting the configured attempts', async () => {
      ...
      await expect(listReleaseAssets(config, releaser, releaseToInspect, 2)).rejects.toThrow('Too many retries.');
      expect(listReleaseAssetsSpy).toHaveBeenCalledTimes(2);
    });
  });
  ...
  describe('GitHubReleaser', () => {
    ...
    it.each([
      ...
      expect(request).toHaveBeenCalledWith('DELETE /repos/{owner}/{repo}/releases/{release_id}/assets/{asset_id}', {
        owner: 'owner',
        repo: 'repo',
        release_id: 42,
        asset_id: 99,
    ...
    it('reports both failures when release-scoped deletion also fails', async () => {
      const releaser = new GitHubReleaser({
        rest: {
          repos: {
            deleteReleaseAsset: vi.fn().mockRejectedValue({ status: 404, message: 'standard route not found' }),
          },
        },
        request: vi.fn().mockRejectedValue({ status: 404, message: 'release-scoped route not found' }),
      } as any);
  ...
  describe('error handling', () => {
    it.each([
      ...
      expect(log).not.toHaveBeenCalledWith(expect.stringContaining('Unexpected error fetching GitHub release'));
    });
    ...
    it('draft-firsts prereleases when files are provided (immutable-safe)', async () => {
      const prereleaseConfig = {
        ...config,
        input_prerelease: true,
        input_draft: false,
    ...
    ])('does not create a replacement after an existing-release update returns %s', async (_name, updateError) => {
      const existingRelease: Release = {
        id: 41,
        upload_url: 'existing-upload',
        html_url: 'existing-html',
    ...
    it.each([
      ...
      const releaseByTag = refreshedCreatedRelease.draft ? canonicalRelease : refreshedCreatedRelease;
      let created = false;
      const deleteRelease = vi.fn().mockResolvedValue(undefined);
      const releaser = createReleaser({
        getReleaseByTag: vi.fn(() =>
  ...
  describe('upload', () => {
    it('restores a dotfile label when GitHub normalizes the uploaded asset name', async () => {
      ...
      try {
        const result = await upload(config, releaser, 'https://uploads.example.test/assets', dotfilePath, [], 1);
