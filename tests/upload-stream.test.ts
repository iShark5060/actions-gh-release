...
describe('release asset upload transport', () => {
  ...
  it.each([0, 1, 2, 3, 4, 5])('uploads fixture %i through the real Octokit request path', async (fixtureIndex) => {
    ...
    try {
      ...
      expect(new Set(receipts[0].chunkTypes)).toEqual(fixture.size === 0 ? new Set() : new Set(['Buffer']));
  ...
  it('closes the file handle when the request fails', async () => {
    ...
    try {
      await expect(upload(config, releaser, uploadUrl, fixture.path, [], 1)).rejects.toThrow('Validation Failed');
      await expectLastFileHandleClosed();
    } finally {
      await closeServer(server);
    }
  ...
  it('opens a fresh upload body after an already-exists response', async () => {
    const fixture = fixtures[0];
    const { server, uploadUrl, receipts } = await startUploadServer((index) => (index === 0 ? 422 : 201));
    const releaser = new GitHubReleaser(getOctokit(config.github_token));
    vi.spyOn(releaser, 'listReleaseAssets').mockResolvedValue([{ id: 9, name: fixture.name }]);
    const deleteReleaseAsset = vi.spyOn(releaser, 'deleteReleaseAsset').mockResolvedValue(undefined);
    openFile.mockClear();
